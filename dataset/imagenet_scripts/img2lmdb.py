...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_path', type=str, default='/raid/huangsh/imagenet/tiny-imagenet-200/images', help='the path of the images')
    parser.add_argument('--list_path', type=str, default='/raid/huangsh/imagenet/tiny-imagenet-200/lmdb', help='the path of the image list')
    parser.add_argument('--output_path', type=str, default='/raid/huangsh/imagenet/tiny-imagenet-200/lmdb', help='the output path of the lmdb file')
    parser.add_argument('--split', type=str, default='val', help='the split path of the images: train / val')
    args = parser.parse_args()

    image_size = args.image_size if args.image_size else image_size
    image_folder = osp.join(args.image_path, args.split)
    image_list = osp.join(args.list_path, '{}_datalist.txt'.format(args.split))

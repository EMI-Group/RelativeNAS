...
class ImageNet12(object):
    ...
    def __init__(self, trainFolder, testFolder, num_workers=8, pin_memory=True, size_images=224,
                 scaled_size=256, type_of_data_augmentation='rand_scale', data_config=None):
    ...
    def _getTransformList(self, aug_type):
        ...
        if aug_type == 'validation':
            ...
            if self.data_config.color:
                list_of_transforms.append(transforms.ColorJitter(brightness=0.4, contrast=0.4,  saturation=0.4, hue=0.2))
    ...
    def _getTrainSet(self):
        ...
        if self.data_config.train_data_type == 'img':
            ...
            train_set = lmdb_dataset.ImageFolder(self.trainFolder, os.path.join(self.trainFolder, '..', 'train_datalist.txt'),
                                                 train_transform, patch_dataset=self.patch_dataset)
    ...
    def _getTestSet(self):
        ...
        if self.data_config.val_data_type == 'img':
            ...
            test_set = lmdb_dataset.ImageFolder(self.testFolder, os.path.join(self.testFolder, '..', 'val_datalist.txt'), test_transform)
            self.test_num_examples = test_set.__len__()
        return test_set


    def getTrainLoader(self, batch_size, shuffle=True, dis_sampler=False, world_size=None, rank=None):
        ...
        if dis_sampler:
            ...
            train_sampler = None 
        train_loader = torch.utils.data.DataLoader(train_set, batch_size=batch_size, shuffle=shuffle, num_workers=0, pin_memory=True, sampler=train_sampler, collate_fn=fast_collate)
        return train_loader

    def getTestLoader(self, batch_size, shuffle=False, dis_sampler=False, world_size=None, rank=None):
        test_set = self._getTestSet()
        if dis_sampler:
            shuffle=False
            val_sampler = torch.utils.data.distributed.DistributedSampler(test_set, num_replicas=world_size, rank=rank)
        else:
            val_sampler = None 
        test_loader = torch.utils.data.DataLoader(test_set, batch_size=batch_size, shuffle=shuffle, num_workers=0, pin_memory=True, sampler=val_sampler, collate_fn=fast_collate)
        return test_loader


    def getTrainTestLoader(self, batch_size, train_shuffle=True, val_shuffle=False, dis_sampler=False, world_size=None, rank=None):
        train_loader = self.getTrainLoader(batch_size, train_shuffle, dis_sampler=dis_sampler, world_size=world_size, rank=rank)
        test_loader = self.getTestLoader(batch_size, val_shuffle, dis_sampler=dis_sampler, world_size=world_size, rank=rank)
